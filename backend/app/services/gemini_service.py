...
class GeminiService:
    ...
    async def analyze_paper_with_gemini(self, content: str, title: str = "") -> Dict[str, Any]:
        """
...
JSON array:"""
    ...
    async def generate_additional_concept_with_gemini(self, content: str, existing_concepts: List[str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            existing_list = "\n".join([f"- {concept}" for concept in existing_concepts]) if existing_concepts else "- None yet"
...
JSON object:"""
            ...
            if response:
                try:
                    ...
                    if '{' in response and '}' in response:
                        ...
                        if name and description and len(name) > 3 and len(description) > 10:
                            # More lenient duplicate checking - allow variations
                            name_lower = name.lower()
                            is_too_similar = any(
                                existing.lower() == name_lower or
                                (len(existing) > 5 and existing.lower() in name_lower and len(name_lower) - len(existing.lower()) < 3)
    ...
    async def generate_manim_code_with_gemini(self, concept_name: str, concept_description: str, paper_title: str = "") -> str:
        ...
        try:
            ...
            safe_name = ''.join(c for c in concept_name.replace(' ', '').replace('-', '') if c.isalnum())[:15]
...
JSON:"""
                ...
                try:
                    ...
                    if '{' in response and '}' in response:
                        ...
                        return {
                            ...
                            "abstract": metadata.get("abstract", "")[:800]  # Limit abstract length
                        }
    ...
    def _parse_gemini_concepts(self, gemini_text: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if '[' in gemini_text and ']' in gemini_text:
                ...
                concepts = []
                for concept in concepts_data[:3]:  # Limit to 3 concepts for initial analysis
    ...
    def _clean_manim_code(self, code_text: str, concept_name: str) -> str:
        ...
        if 'class ' not in code_text or 'Scene' not in code_text:
            return self._generate_fallback_manim_code(concept_name, "Key research concept")
    ...
    def _generate_fallback_manim_code(self, concept_name: str, concept_description: str) -> str:
        ...
        short_desc = concept_description[:80] if len(concept_description) > 80 else concept_description
...
'''
    ...
    async def _fallback_analysis(self, content: str, title: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "insights": ["Basic analysis completed. For better results, check Gemini API connection."],
